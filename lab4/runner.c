...
static void *threadfn(void *datav)
{
    ...
    FAIL_IF(real_read_size != op_size,
            "zc_read returned wrong size - expected %zu, got %zu\n", op_size,
            real_read_size);
    FAIL_IF(memcmp(read, randdata + op_size * 3, op_size),
...
int main(int argc, char *argv[])
{
    ...
    RUNNER_ERROR_IF(pthread_barrier_init(&barrier, NULL, 2),
    ...
    RUNNER_ERROR_IF(snprintf(path1, sizeof(path1), "/tmp/zc_io1_%d_%ld",
                             getpid(), lrand48()) < 0,
                    "snprintf failed\n");
    RUNNER_ERROR_IF(snprintf(path2, sizeof(path2), "/tmp/zc_io2_%d_%ld",
                             getpid(), lrand48()) < 0,
                    "snprintf failed\n");
    // file3 in the current directory (we can't use tmpfs if we want to observe
    // caching)
    RUNNER_ERROR_IF(snprintf(path3, sizeof(path3), "zc_io3_%d_%ld", getpid(),
                             lrand48()) < 0,
                    "snprintf failed\n");
    ...
    eprintf(
        "test 2a - write to existing file, overwriting existing contents\n");
    {
        const size_t size = GEN_SIZE();
        FILL_FILE(file1, size);
        eprintf("filling file with %zu bytes\n", size);
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            FAIL_IF(fread(scratch, 1, sizeof(scratch), file1) !=
                        expected_file_size,
            ...
            FAIL_IF(memcmp(scratch, randdata + 1048576, offset) ||
                    ...
                    "zc_write failed - wrong contents seen in file after "
                    "zc_write_end\n");
        }
    ...
    eprintf("test 2b - write to new file\n");
    {
        ...
        FAIL_IF(!file2,
                "could not open %s, did zc_open forget to pass an appropriate "
                "mode to open?\n",
                path2);
...
#define TEST3_WRITE(_____sz)                                                   \
    ...
    {                                                                          \
        ...
        FAIL_IF(fread(scratch, 1, sizeof(scratch), file1) != size,             \
                "fread returned wrong size - did zc_write cause file to have " \
                "wrong length?\n");                                            \
...
#define TEST3_SEEK(f, o, s)                                                 \
    ...
    eprintf("test 4 - copy\n");
    {
        ...
        FILE *file2 = fopen(path2, "r");
        FAIL_IF(!file2,
                "could not open %s, did zc_copyfile forget to pass an "
                "appropriate mode to open?\n",
                path2);
        FAIL_IF(fread(scratch, 1, size, file2) != size,
                "fread returned wrong size for file2\n");
        fclose(file2);
        file2 = NULL;
...
#define TEST5_WAITUNTIL(cond, ms)                                    \
    ...
    eprintf("test 5 - synchronisation\n"
            ...
            "*** you probably have a synchronisation problem\n");
    {
        ...
        FAIL_IF(real_read_size != op_size,
                "zc_read returned wrong size - expected %zu, got %zu\n",
                op_size, real_read_size);
        FAIL_IF(memcmp(read1, randdata, op_size),
                "zc_read returned wrong contents\n");
        FAIL_IF(memcmp(read2, randdata + op_size, op_size),
                "zc_read returned wrong contents\n");
        zc_read_end(zcfile);
        zc_read_end(zcfile);

        {
            ...
            *thread_data =
                (struct thread_data){.file = zcfile, .op_size = op_size};
        ...
        FAIL_IF(real_read_size != op_size,
                "zc_read returned wrong size - expected %zu, got %zu\n",
                op_size, real_read_size);
        FAIL_IF(memcmp(read1, randdata + op_size * 2, op_size),
                "zc_read returned wrong contents\n");
        // barrier 1, then they acquire
        pthread_barrier_wait(&barrier);
        // wait ~1s for them to indicate that they have acquired
        TEST5_WAITUNTIL(thread_counter > 0, 1000);
        ...
        FAIL_IF(real_read_size != op_size,
                "zc_read returned wrong size - expected %zu, got %zu\n",
                op_size, real_read_size);
        FAIL_IF(memcmp(read1, randdata + op_size * 4, op_size),
                "zc_read returned wrong contents\n");
        // barrier 3, then they acquire
        pthread_barrier_wait(&barrier);
        // wait 500ms, be reasonably sure they are blocked
        TEST5_WAITUNTIL(thread_counter > 0, 500);
        FAIL_IF(thread_counter,
                "worker thread acquired write while we are reading!\n");
        // release the read
        zc_read_end(zcfile);
        ...
        TEST5_WAITUNTIL(thread_counter > 0, 500);
        FAIL_IF(thread_counter,
                "worker thread acquired write while we are writing!\n");
        // release the read
        zc_write_end(zcfile);
        // now wait until they get the write
        TEST5_WAITUNTIL(thread_counter > 0, 1000);
